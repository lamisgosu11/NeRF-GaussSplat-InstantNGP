...
import numpy as np
from utils.camera import parse_camera


def get_rays_single_image(H, W, intrinsics, c2w, render_stride=1):
    device = intrinsics.device
    # indexing = x, y
    u, v = np.meshgrid(np.arange(W)[:: render_stride], np.arange(H)[
                       :: render_stride])
    ...
    rays_d = (
        c2w[:, :3, :3].bmm(torch.inverse(
            intrinsics[:, :3, :3])).bmm(batched_pixels)
    ).transpose(1, 2)
...
def get_point_clouds(cameras, depths, alphas, rgbs=None):
    ...
    coords = []
    rgbas = []
    rays_o, rays_d = get_rays_single_image(
        H=H, W=W, intrinsics=intrinsics, c2w=c2ws)
