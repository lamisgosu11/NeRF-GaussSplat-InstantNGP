...
def to_viewpoint_camera(camera):
    ...
    camera = Camera(width=int(Ws[0]), height=int(
        Hs[0]), intrinsic=intrinsics[0], c2w=c2ws[0])
    return camera
...
class Camera(nn.Module):
    def __init__(self, width, height, intrinsic, c2w, znear=0.1, zfar=100., trans=np.array([0.0, 0.0, 0.0]), scale=1.0):
        ...
        self.projection_matrix = getProjectionMatrix(
            znear=self.znear, zfar=self.zfar, fovX=self.FoVx, fovY=self.FoVy).transpose(0, 1).to(device)
