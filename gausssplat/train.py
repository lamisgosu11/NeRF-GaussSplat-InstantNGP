...
from utils.points import get_point_clouds
from models.gauss_model import GaussModel
from models.render import GaussRenderer
from data_utils.loader import read_all
...
class GSSTrainer(Trainer):
    ...
    def on_train_step(self):
        ...
        if USE_PROFILE:
            print(prof.key_averages(group_by_stack_n=True).table(
                sort_by='self_cuda_time_total', row_limit=20))

        l1_loss = loss_utils.l1_loss(out['render'], rgb)
        depth_loss = loss_utils.l1_loss(
            out['depth'][..., 0][mask], depth[mask])
        ssim_loss = 1.0-loss_utils.ssim(out['render'], rgb)

        total_loss = (1-self.lambda_dssim) * l1_loss + \
            self.lambda_dssim * ssim_loss + depth_loss * self.lambda_depth
        psnr = utils.img2psnr(out['render'], rgb)
        log_dict = {'total': total_loss, 'l1': l1_loss,
                    'ssim': ssim_loss, 'depth': depth_loss, 'psnr': psnr}
    ...
    def on_evaluate_step(self, **kwargs):
        ...
        utils.imwrite(str(self.results_folder /
                      f'image-{self.step}.png'), image)


if __name__ == "__main__":
    device = 'cuda'
    folder = '/gausssplat/data'
    ...
    points = get_point_clouds(
        data['camera'], data['depth'], data['alpha'], data['rgb'])
