...
def read_image(rgb_file, pose, intrinsic_, max_depth, resize_factor=1., white_bkgd=True):
    ...
    depth = torch.from_numpy(imageio.imread(
        rgb_file[:-7]+'depth.png').astype(np.float32) / 255.0 * max_depth)
    alpha = torch.from_numpy(imageio.imread(
        rgb_file[:-7]+'alpha.png').astype(np.float32) / 255.0)
    ...
    if resize_factor != 1:
        image_size = image_size[0] * \
            resize_factor, image_size[1] * resize_factor
        intrinsic[:2, :3] *= resize_factor

        def resize_fn(img, resize_factor): return F.interpolate(
            img.permute(0, 3, 1, 2), scale_factor=resize_factor, mode='bilinear',
        ).permute(0, 2, 3, 1)

        rgb = rearrange(resize_fn(
            rearrange(rgb, 'h w c -> 1 h w c'), resize_factor), '1 h w c -> h w c')
        depth = rearrange(
            resize_fn(rearrange(depth, 'h w -> 1 h w 1'), resize_factor), '1 h w 1 -> h w')
        alpha = rearrange(
            resize_fn(rearrange(alpha, 'h w -> 1 h w 1'), resize_factor), '1 h w 1 -> h w')
